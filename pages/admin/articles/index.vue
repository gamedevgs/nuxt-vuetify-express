            ...
            <v-layout row wrap>
              <v-flex lg9 class="pa-1">
                ...
                <v-text-field
                  ...
                  required
                  :rules="[
                    (v) => !!v || 'Tóm tắt không được để trống',
              ...
              <v-flex lg3 class="pa-2">
                ...
                <v-toolbar card color="white">
                  <v-btn
                    v-if="popup.head_title === 'Tạo Bài Viết'"
                    color="success"
                    round
                    @click="submitForm"
                  >
...
export default {
  ...
  methods: {
    ...
    AddArticle() {
      ...
      this.popup.head_title = "Tạo Bài Viết";
    ...
    async ReviewArticle(id) {
      const { data } = await this.$axios.get("/api/articles/" + id);
      this.review = data;
    ...
    async UpdateCatId(id) {
      this.popup.id_x = id;
      this.popup.dialog = true;
      this.popup.head_title = "Sửa Bài Viết";
      this.defaulThumb = require("@/static/discover/ds_5.jpeg");
      this.popup.tags = [];
      const { data } = await this.$axios.get("/api/articles/" + id);
          ...
          this.$axios
            ...
            .then((response) => {
              if (response.data._id) {
                ...
                this.updateAllArticle();
                this.makeToast("success", "Thêm Bài Viết thành công");
              }
            })
            .catch((error) => {
    ...
    UpdateForm() {
      ...
      this.$axios
        .put("/api/articles/" + this.popup.id_x, obj)
        .then((response) => {
          if (response.data._id) {
            this.popup.dialog = false;
            this.updateAllArticle();
            this.makeToast("success", "Cập nhật Bài Viết thành công");
          }
        })
        .catch((error) => {
          if (error.response.data.errors[0].msg) {
      ...
      if (confirm("Bạn muốn xóa Danh Mục này?") === true) {
        this.$axios
          ...
          .then((response) => {
            if (response.data._id) {
              this.updateAllArticle();
              this.makeToast("success", "Xóa Bài Viết thành công");
            }
          })
          .catch((error) => {
            console.log(error);
  ...
  head() {
    return {
      ...
      meta: [
        ...
        {
          hid: "descrissspsssssssssstion",
          name: "ssssssssssss",
          content: "My custom sssssssssssssssss",
        },
      ],
    };
  },
